/*
  # Comprehensive Site Audit Log

  1. Purpose
    - Consolidate all site-related activity into unified audit log
    - Include device events, alerts, commands, images, assignments
    - Provide complete visibility into site operations

  2. Data Sources Integrated
    - pilot_program_history_staging (site updates, submissions, observations)
    - device_history (device lifecycle events)
    - device_alerts (alerts triggered/resolved)
    - device_commands (user commands to devices)
    - device_images (image captures and MGI scoring)
    - device_site_assignments (device assignment changes)
    - device_schedule_changes (wake schedule updates)

  3. Deduplication Strategy
    - device_history is primary source for device events
    - Exclude high-volume telemetry data
...
-- ============================================
-- FUNCTION: Comprehensive Site Audit Log
-- ============================================
...
COMMENT ON FUNCTION get_comprehensive_site_audit_log IS
  'Returns comprehensive audit log for a site including all device activity, alerts, commands, images, and user actions';
