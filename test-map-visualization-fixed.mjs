...
const { data: sites, error: sitesError } = await supabase
  .from('sites')
  .select('site_id, name, length, width, site_template_id')
  .not('length', 'is', null)
  .not('width', 'is', null)
  .limit(5);
...
if (sessionsError) {
  ...
  for (const session of activeSessions) {
    console.log(`   Session: ${session.session_id.slice(0, 8)}...`);
    ...
    if (snapshotCount && snapshotCount > 0) {
      ...
      if (latestSnapshot) {
        ...
        if (siteState && Array.isArray(siteState)) {
          const devicesWithPositions = siteState.filter(d =>
            d.position && d.position.x !== null && d.position.y !== null
          );
          console.log(`   Devices in snapshot: ${siteState.length}`);
...
if (devError) {
  ...
  devicesWithPos.slice(0, 5).forEach(d => {
...
if (testSession) {
  console.log(`Testing session: ${testSession.session_id.slice(0, 8)}...`);
  ...
  // Check 2: Devices exist with positions
  ...
  // Check 4: Environmental data available
...
if (allPassed) {
  ...
  console.log('\nThe map visualization flow is complete:');
