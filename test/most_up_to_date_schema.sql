...
CREATE TABLE public.device_telemetry (
  ...
  CONSTRAINT device_telemetry_wake_payload_id_fkey FOREIGN KEY (wake_payload_id) REFERENCES public.device_wake_payloads(payload_id)
);
CREATE TABLE public.device_wake_payloads (
  ...
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  telemetry_id uuid,
  wake_type text CHECK (wake_type = ANY (ARRAY['image_wake'::text, 'telemetry_only'::text, 'hello'::text, 'retry'::text])),
  ...
  CONSTRAINT device_wake_payloads_telemetry_id_fkey FOREIGN KEY (telemetry_id) REFERENCES public.device_telemetry(telemetry_id)
);
...
CREATE TABLE public.duplicate_images_log (
  log_id uuid NOT NULL DEFAULT gen_random_uuid(),
  device_id uuid NOT NULL,
  image_id uuid,
  image_name text NOT NULL,
  original_captured_at timestamp with time zone,
  duplicate_received_at timestamp with time zone NOT NULL DEFAULT now(),
  duplicate_metadata jsonb NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT duplicate_images_log_pkey PRIMARY KEY (log_id),
  CONSTRAINT duplicate_images_log_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.devices(device_id),
  CONSTRAINT duplicate_images_log_image_id_fkey FOREIGN KEY (image_id) REFERENCES public.device_images(image_id)
);
...
CREATE TABLE public.petri_observations (
  ...
  CONSTRAINT petri_observations_device_id_fkey FOREIGN KEY (device_id) REFERENCES public.devices(device_id)
);
...
CREATE TABLE public.roi_calculations (
  ...
  facility_sqft numeric NOT NULL CHECK (facility_sqft > 0::numeric),
  annual_production numeric NOT NULL CHECK (annual_production > 0::numeric),
  waste_percent numeric NOT NULL CHECK (waste_percent >= 0::numeric AND waste_percent <= 100::numeric),
  downtime_days numeric NOT NULL CHECK (downtime_days >= 0::numeric AND downtime_days <= 365::numeric),
  ...
  max_roi numeric,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT roi_calculations_pkey PRIMARY KEY (calculation_id)
);
...
CREATE TABLE public.sites (
  ...
  snapshot_cadence_per_day integer NOT NULL DEFAULT 3 CHECK (snapshot_cadence_per_day = ANY (ARRAY[1, 3, 6, 12, 24])),
  snapshot_cadence_hours integer DEFAULT 3 CHECK (snapshot_cadence_hours >= 1 AND snapshot_cadence_hours <= 24),
