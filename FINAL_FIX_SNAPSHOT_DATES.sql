/*
  # FINAL FIX: Explicit Interval Casting for Date Calculations

  The issue: PostgreSQL can't infer the type when subtracting timestamps.

  Solution: Cast the result explicitly to interval, then extract days.
*/
...
BEGIN
  ...
  -- Calculate program days EXPLICITLY first
  SELECT
    EXTRACT(DAY FROM (p_wake_round_end - pp.start_date))::integer,
    EXTRACT(DAY FROM (pp.end_date - pp.start_date))::integer
  INTO v_program_day, v_total_days
...
BEGIN
  ...
  RAISE NOTICE '✅ SUCCESS! Snapshot ID: %', test_result;
