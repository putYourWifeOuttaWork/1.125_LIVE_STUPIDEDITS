/*
  # Fix EXTRACT Bug in generate_session_wake_snapshot

  ## Problem
  Function fails with: "function pg_catalog.extract(unknown, integer) does not exist"

  ## Root Cause
  Lines 281-282 use:
    EXTRACT(EPOCH FROM (timestamp - timestamp)) / 86400

  But timestamp subtraction returns an interval, and the syntax is confusing PostgreSQL.

  ## Solution
  Use simpler: EXTRACT(DAY FROM interval) which directly gets days from the interval
*/
...
BEGIN
  ...
  DO UPDATE SET
    ...
    new_images_count = EXCLUDED.new_images_count,
    new_alerts_count = EXCLUDED.new_alerts_count,
    avg_temperature = EXCLUDED.avg_temperature,
    avg_humidity = EXCLUDED.avg_humidity,
    avg_mgi_score = EXCLUDED.avg_mgi_score,
    max_mgi_score = EXCLUDED.max_mgi_score,
    updated_at = now()
...
'Generate a snapshot of the site state for a specific wake round, including device connectivity metrics';

-- Test the fix
DO $$
BEGIN
  RAISE NOTICE '✅ Function recreated with EXTRACT(DAY FROM interval) syntax';
  RAISE NOTICE '📝 Now run: node regenerate-snapshots-with-locf.mjs';
END $$;
