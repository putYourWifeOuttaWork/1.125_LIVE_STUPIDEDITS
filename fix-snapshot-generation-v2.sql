...
-- 4. Fixed nested aggregates in zone_environment CTE
-- 5. Store aggregate values before using them outside WITH clause
...
BEGIN
  ...
  -- Compute MGI aggregates (store before WITH clause)
  SELECT
    AVG(di.mgi_score),
    MAX(di.mgi_score)
  ...
  -- Compute environmental aggregates (store before WITH clause)
  SELECT
    AVG(dt.temperature),
    AVG(dt.humidity)
  ...
  -- Environmental aggregates summary
  env_aggregates AS (
    SELECT
...
COMMENT ON FUNCTION generate_session_wake_snapshot IS 'Generate complete JSONB snapshot of site state after wake round completes. Includes all devices, MGI metrics, telemetry, zones, and alerts. Fixed to use correct device_alerts columns and avoid CTE scope issues.';
