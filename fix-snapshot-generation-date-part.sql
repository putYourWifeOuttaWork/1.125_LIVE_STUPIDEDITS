-- Fix DATE_PART issues in generate_session_wake_snapshot function
-- The issue: DATE_PART('day', timestamp - timestamp) needs explicit interval casting
...
COMMENT ON FUNCTION generate_session_wake_snapshot IS 'Generate complete JSONB snapshot of site state after wake round completes. Includes all devices, MGI metrics, telemetry, zones, and alerts. Fixed DATE_PART issues.';
