/*
  # Simple Snapshot Generation Fix

  CRITICAL: This replaces the complex function with a SIMPLE, WORKING version.

  INSTRUCTIONS:
  1. Open Supabase Dashboard → SQL Editor
  2. Copy this ENTIRE file
  3. Paste and click RUN
  4. You should see "SUCCESS: Simple snapshot function deployed!"

  This simplified version:
  - Queries device_wake_payloads for telemetry (temperature, humidity)
  - Includes device positions in site_state
  - Works with existing wake_payload_id foreign keys
  - No dependencies on helper functions that don't exist
*/
...
BEGIN
    ...
    'devices', (
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'mgi_state', (
            SELECT jsonb_build_object(
              'latest_mgi_score', di.mgi_score,
              'mgi_velocity', jsonb_build_object(
                'per_hour', di.mgi_velocity
              ),
              'scored_at', di.scored_at
            )
  ...
  -- Calculate MGI aggregates from images via wake_payloads
  SELECT
    AVG(di.mgi_score),
    MAX(di.mgi_score)
...
-- Add helpful comment
COMMENT ON FUNCTION generate_session_wake_snapshot IS 'Simple snapshot generation using device_wake_payloads. No external dependencies.';

-- Grant permissions
GRANT EXECUTE ON FUNCTION generate_session_wake_snapshot(uuid, integer, timestamptz, timestamptz) TO authenticated;
GRANT EXECUTE ON FUNCTION generate_session_wake_snapshot(uuid, integer, timestamptz, timestamptz) TO service_role;

-- Success message
SELECT 'SUCCESS: Simple snapshot function deployed!' as status;
