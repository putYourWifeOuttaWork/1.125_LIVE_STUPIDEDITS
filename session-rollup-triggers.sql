/*
  # Session Roll-Up Counter Triggers

  ## Purpose
  Create database triggers to automatically maintain roll-up counters on site_device_sessions.
  These counters track wake completion, failures, and extra wakes per session.

  ## Changes

  ### 1. Function: increment_session_wake_counts()
  - Fires on INSERT/UPDATE to device_wake_payloads
  - Increments completed_wake_count when wake_complete = true
  - Increments failed_wake_count when wake_failed = true
  - Increments extra_wake_count when is_extra_wake = true
  ...
  - site_device_sessions table
  - Phase 2 wake payload consolidation
...
BEGIN
  ...
  v_is_complete := COALESCE(NEW.wake_complete, FALSE);
  v_is_failed := COALESCE(NEW.wake_failed, FALSE);
  v_is_extra := COALESCE(NEW.is_extra_wake, FALSE);

  -- For UPDATE operations, check if status changed
  IF TG_OP = 'UPDATE' THEN
    -- If wake_complete status changed from FALSE to TRUE
    IF v_is_complete = TRUE AND COALESCE(OLD.wake_complete, FALSE) = FALSE THEN
    ...
    -- If wake_failed status changed from FALSE to TRUE
    IF v_is_failed = TRUE AND COALESCE(OLD.wake_failed, FALSE) = FALSE THEN
      UPDATE site_device_sessions
      SET
    ...
    -- If is_extra_wake status changed from FALSE to TRUE
    IF v_is_extra = TRUE AND COALESCE(OLD.is_extra_wake, FALSE) = FALSE THEN
      UPDATE site_device_sessions
      SET
  ...
  ELSIF TG_OP = 'INSERT' THEN
    ...
    SET
      completed_wake_count = CASE
        WHEN v_is_complete THEN COALESCE(completed_wake_count, 0) + 1
        ELSE COALESCE(completed_wake_count, 0)
      END,
      failed_wake_count = CASE
        WHEN v_is_failed THEN COALESCE(failed_wake_count, 0) + 1
        ELSE COALESCE(failed_wake_count, 0)
      END,
      extra_wake_count = CASE
        WHEN v_is_extra THEN COALESCE(extra_wake_count, 0) + 1
...
CREATE TRIGGER trg_increment_session_wake_counts
  AFTER INSERT OR UPDATE OF wake_complete, wake_failed, is_extra_wake
