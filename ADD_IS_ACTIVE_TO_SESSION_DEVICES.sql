/*
  ...
  Run this in Supabase SQL Editor to update the get_session_devices_with_wakes function
  to return is_active status for each device.

  This fixes the "Inactive" status showing for all devices in Device Performance Summary.
*/
...
BEGIN
  ...
  LOOP
    ...
    v_device_array := v_device_array || jsonb_build_object(
      ...
      'is_active', v_device_record.is_active,    -- ⭐ ADDED THIS LINE
