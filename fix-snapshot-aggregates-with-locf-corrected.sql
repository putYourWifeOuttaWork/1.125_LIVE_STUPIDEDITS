/*
  # Fix Snapshot Aggregates with LOCF (Last Observation Carried Forward) - CORRECTED

  ## Problem
  - Temperature/humidity telemetry only exists for early wakes (hour 1)
  - Later wakes have images but no telemetry records
  - Snapshot aggregates were NULL for most wakes
  - Original fix had date arithmetic error with DATE_PART

  ## Solution
  - Use LOCF: Query the most recent telemetry BEFORE the wake window
  - Carry forward environmental data from previous wakes
  - Keep MGI data from actual wake window (images)
  - Fix date arithmetic for program_day and total_days

  ## Changes
  - Updated generate_session_wake_snapshot function to use LOCF for telemetry
  - Fixed DATE_PART to work with proper date/interval arithmetic
  - Environmental data now propagates forward until new data arrives
*/
...
BEGIN
  ...
  program_meta AS (
    SELECT jsonb_build_object(
      ...
      'program_day', (p_wake_round_end::date - pp.start_date)::integer,
      'total_days', (pp.end_date - pp.start_date)::integer
    ) AS program_context
...
COMMENT ON FUNCTION generate_session_wake_snapshot IS 'Generate complete JSONB snapshot of site state after wake round completes. Uses LOCF (Last Observation Carried Forward) for environmental data when telemetry is missing in the wake window.';
