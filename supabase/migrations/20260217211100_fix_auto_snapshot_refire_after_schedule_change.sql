/*
  # Fix auto-snapshot not firing after schedule time change

  1. Problem
    - When a user changes the snapshot_time on an existing schedule that already
      ran today, the cadence gate (days_elapsed < min_days) blocks the new run
      because it only checks calendar dates, not whether the schedule was modified.

  2. Fix
    - Add a bypass condition: if `updated_at > last_run_at`, the user modified
      the schedule since the last run, so skip the cadence gate.
    - After each successful snapshot, the function already sets both
      `last_run_at = now()` and `updated_at = now()`, so the bypass self-resets.

  3. Changes
    - Replaces `generate_scheduled_report_snapshots()` with one additional
      condition on the cadence skip: `AND v_schedule.updated_at <= v_schedule.last_run_at`

  4. Impact
    - No schema changes
    - No data changes
    - Only affects the cadence gating logic in the cron-called function
*/

CREATE OR REPLACE FUNCTION public.generate_scheduled_report_snapshots()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
