/*
  # Fix View Security Context for pilot_programs_with_progress

  1. Purpose
    - Ensure the pilot_programs_with_progress view respects RLS policies
    - Set SECURITY INVOKER to use the calling user's permissions
    - This ensures company admins can see their programs through the view

  2. Problem
    - Views created without SECURITY INVOKER run with the creator's permissions
    - This can bypass RLS policies and cause access issues

  3. Solution
    - Recreate the view with SECURITY INVOKER option
    - This makes the view respect the RLS policies on pilot_programs table
    - Access checks happen in the context of the user querying the view
*/
...
COMMENT ON VIEW pilot_programs_with_progress IS 'View that extends pilot_programs with dynamic progress metrics. Uses SECURITY INVOKER to respect RLS policies.';
