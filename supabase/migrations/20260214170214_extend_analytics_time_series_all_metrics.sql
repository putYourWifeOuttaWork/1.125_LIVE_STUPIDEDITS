/*
  # Extend analytics time series RPC to support all metric types

  1. Changes
    - Adds CASE branches for battery_voltage, mgi_velocity, mgi_speed
    - These columns already exist on device_images and will now return real values
    - wake_reliability, image_success_rate, and alert_count are not stored on
      device_images so they continue to return NULL from this function
      (they require session-level aggregation)
...
BEGIN
  ...
  SELECT
    ...
    CASE m.metric_name
      ...
      WHEN 'battery_voltage' THEN AVG(di.battery_voltage)
