/*
  # Update get_session_devices_with_wakes to include status fields

  1. Modified Functions
    - `get_session_devices_with_wakes(p_session_id UUID)`
      - Now returns `is_active`, `last_wake_at`, `last_seen_at`, `next_wake_at` for each device
      - Adds `last_session_wake_at`: the most recent wake timestamp scoped to the session
      - Adds `missed_wakes_in_session`: count of missed wake windows calculated from
        the last session wake (or session start if no wakes yet) using fn_calculate_missed_wakes
      - Adds `missed_wakes_global`: count of missed wake windows from the device's global last_wake_at

  2. Important Notes
    - This is a CREATE OR REPLACE, safe to apply on existing function
    - No changes to table structure or data
    - Fixes bug where is_active and last_seen_at were missing from response,
      causing all devices to render as "Inactive" in the UI
*/
