/*
  # Fix report auto-snapshot function

  1. Bug Fixes
    - Remove reference to `di.battery_voltage` which does not exist on `device_images`
    - Source battery_voltage from `device_wake_payloads` via LEFT JOIN instead
    - Fix metrics parsing: extract `type` field from metric config objects instead of
      using the full JSON object string (was causing no CASE branch to ever match)
    - Relax `di.status = 'complete'` to include 'receiving' images that already have
      valid telemetry data (data fidelity over data perfection)

  2. Changes
    - `generate_scheduled_report_snapshots` function rewritten with corrected query
    - Metrics now properly extracted as `m->>'type'` from JSONB array of objects
    - battery_voltage sourced from device_wake_payloads via LEFT JOIN
    - Status filter relaxed to ('complete', 'receiving')

  3. Important Notes
    - This fixes the `column di.battery_voltage does not exist` error that has been
      preventing ALL scheduled report snapshots from being generated
    - Both currently configured schedules (daily 07:56 ET and daily 08:00 ET)
      have `last_run_at: NULL` because the function has never succeeded
    - After this fix, the next cron cycle (every 15 minutes) should generate snapshots
*/
