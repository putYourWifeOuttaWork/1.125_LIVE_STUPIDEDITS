/*
  # Add Device Tracking Columns (FIXED)
  ...
  2. New Columns
    ...
    - `last_updated_by_user_id` - Audit trail for changes (NO FOREIGN KEY)
  ...
  5. IMPORTANT FIX
    - Removed REFERENCES auth.users(id) to allow system user UUID
    - System user UUID (00000000-0000-0000-0000-000000000001) is not in auth.users
    - We validate UUIDs in application logic instead
*/

-- ==========================================
-- DROP FOREIGN KEY IF EXISTS (from previous attempt)
-- ==========================================
...
BEGIN
  IF EXISTS (
    ...
    ALTER TABLE devices DROP CONSTRAINT devices_last_updated_by_user_id_fkey;
    RAISE NOTICE 'Dropped foreign key constraint devices_last_updated_by_user_id_fkey';
  END IF;
END $$;
...
-- Audit trail for user changes (NO FOREIGN KEY to allow system user UUID)
ALTER TABLE devices
...
- 00000000-0000-0000-0000-000000000001 for system automated updates
- Actual user UUID from auth.users for user-initiated changes

NOTE: No foreign key constraint to allow system user UUID that is not in auth.users.
Application validates UUIDs against auth.users OR system_users table.';
