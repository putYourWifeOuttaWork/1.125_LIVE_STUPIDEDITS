/*
  # Add auto-pause for inactive program snapshot schedules

  1. New Columns on `report_snapshot_schedules`
    - `paused_reason` (text, nullable) - Human-readable reason when auto-paused
    - `paused_at` (timestamptz, nullable) - When the auto-pause occurred

  2. Updated Function: `generate_scheduled_report_snapshots()`
    - Before generating a snapshot, checks whether ALL referenced programs
      have expired (end_date < CURRENT_DATE)
    - Also checks whether any active devices remain assigned to those programs
    - If all programs expired AND no active devices, auto-pauses the schedule
      by setting enabled = false, paused_reason, and paused_at
    - Records auto-pause in the function return results with status 'auto_paused'
    - Reports with no program filter (programIds is null/empty) are NOT auto-paused

  3. Important Notes
    - Users can manually re-enable a paused schedule at any time
    - The paused_reason field lets the UI show WHY the schedule was paused
    - This prevents generating empty snapshots after programs conclude
*/
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_name = 'report_snapshot_schedules' AND column_name = 'paused_reason'
  ) THEN
    ALTER TABLE report_snapshot_schedules ADD COLUMN paused_reason text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'report_snapshot_schedules' AND column_name = 'paused_at'
  ) THEN
    ALTER TABLE report_snapshot_schedules ADD COLUMN paused_at timestamptz;
  END IF;
END $$;
