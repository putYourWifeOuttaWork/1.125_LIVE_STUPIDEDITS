/*
  # Fix auto-snapshot cadence drift and missing metric data

  1. Bug Fix: Scheduling Drift
    - Replaced elapsed wall-clock time cadence check with calendar-date arithmetic
    - Old logic: `(now() - last_run_at) < interval '1 day'` caused snapshots to drift
      later each day because the interval was measured from the actual run time, not
      the target date boundary
    - New logic: computes `v_days_elapsed` as the difference in calendar dates
      (in the schedule's timezone) and compares against the cadence threshold
    - Daily snapshots now fire on the first cron cycle on or after the target time,
      regardless of when the previous day's snapshot actually ran

  2. Bug Fix: Missing Metrics (pressure, gas_resistance)
    - Added `WHEN 'pressure' THEN di.pressure` to the AVG CASE expression
    - Added `WHEN 'gas_resistance' THEN di.gas_resistance` to the AVG CASE expression
    - These metrics were added to `get_analytics_time_series` (the live RPC) in
      migration 20260215192656 but the auto-snapshot inline query was never updated
    - Result: all auto-snapshots for reports using pressure or gas_resistance were
      producing rows with NULL metric_value (blank charts)

  3. Data Sources
    - mgi_score, temperature, humidity, pressure, gas_resistance, mgi_velocity,
      mgi_speed: all sourced from `device_images` (di.*)
    - battery_voltage: sourced from `device_wake_payloads` via LEFT JOIN on
      `di.wake_payload_id` (not available on device_images)

  4. Important Notes
    - No schema changes, no RLS changes, no cron schedule changes
    - Existing blank auto-snapshots in the database are not retroactively fixed
    - The fix takes effect on the next 15-minute cron cycle after deployment
*/
  ...
  LOOP
    BEGIN
      ...
      IF v_schedule.last_run_at IS NOT NULL THEN
        ...
        IF v_days_elapsed < v_min_days THEN
          v_skipped_count := v_skipped_count + 1;
          CONTINUE;
        END IF;
      END IF;
      ...
      FROM (
        ...
        LEFT JOIN device_wake_payloads wp ON wp.wake_payload_id = di.wake_payload_id
