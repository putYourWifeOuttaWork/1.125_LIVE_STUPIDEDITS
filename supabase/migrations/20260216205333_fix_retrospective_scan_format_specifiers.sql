/*
  # Fix format() specifiers in fn_retrospective_mgi_scan

  1. Changes
    - Replaces invalid C-style format specifiers (%.2f, %.4f, %.1f) with %s
    - Uses round() to format numeric values before passing to format()
    - Postgres format() only supports %s, %I, %L -- not %f

  2. Important Notes
    - No data changes, function replacement only
    - Fixes the "unrecognized format() type specifier" error on live scan
*/
