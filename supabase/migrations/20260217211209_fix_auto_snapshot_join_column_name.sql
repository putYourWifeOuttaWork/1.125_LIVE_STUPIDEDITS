/*
  # Fix wake payload join column name in snapshot function

  1. Problem
    - The LEFT JOIN references `wp.wake_payload_id` but the actual column
      in `device_wake_payloads` is `payload_id`.
    - This caused all snapshot generation attempts to fail with:
      "column wp.wake_payload_id does not exist"

  2. Fix
    - Change JOIN condition from `wp.wake_payload_id = di.wake_payload_id`
      to `wp.payload_id = di.wake_payload_id`

  3. Impact
    - Fixes snapshot generation for all reports
    - Includes the cadence bypass fix from previous migration
*/
