-- ================================================
-- NUCLEAR OPTION: Complete function recreation
-- ================================================

-- Step 1: Terminate any active connections using this function (optional, careful!)
-- SELECT pg_terminate_backend(pid) FROM pg_stat_activity 
-- WHERE state = 'active' AND query LIKE '%generate_session_wake_snapshot%';

-- Step 2: Drop ALL versions with CASCADE
DO $$
BEGIN
  ...
  RAISE NOTICE 'Dropped all versions of function';
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Drop failed or no function exists: %', SQLERRM;
END
$$;

-- Step 3: Wait a moment (connection pool refresh)
SELECT pg_sleep(0.5);

-- Step 4: Create fresh function with explicit typing
...
COMMENT ON FUNCTION generate_session_wake_snapshot IS 'Generate comprehensive wake snapshot with LOCF, MGI metrics, device connectivity, and zone analytics. Uses pre-calculated program days.';

-- Verify it was created and show details
SELECT 
  'Function recreated with pre-calculated program days!' as status,
  p.proname as function_name,
  pg_get_function_arguments(p.oid) as arguments,
  p.prosrc LIKE '%v_program_day_calc%' as uses_precalc,
